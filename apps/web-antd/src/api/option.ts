...
export function saveConfigApi(params: SaveConfigParams) {
  ...
  try {
    ...
    // 使用标准格式发送请求
    return authRequestClient.post('/api/v1/sys/option/save-config', formattedParams, {
      headers: {
...
export function getConfigInfoApi() {
  return authRequestClient.get('/api/v1/sys/option/get-sys-config-info');
}
...
export function deleteConfigByIdApi(id: number) {
  return authRequestClient.delete(`/api/v1/sys/option/delete-key-by-id/${id}`);
}

// 获取未加密的API Key
export function getApiKeyById(id: number) {
  return authRequestClient.get<ApiResponse<string>>(`/api/v1/sys/option/get-api-key/${id}`);
}

// 获取配置数据
export function getConfigByApiKey(apiKey: string) {
  return authRequestClient.get<ApiResponse<{ config_data: Record<string, string> }>>('/api/v1/sys/option/get-config', {
    headers: {
      'api-key': apiKey
    }
  });
...
export function updateConfigApi(apiKey: string, configData: Record<string, string>) {
  return authRequestClient.put('/api/v1/sys/option/update-config', 
